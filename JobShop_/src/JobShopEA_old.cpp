...
#include "Parser.h"
  ...
  std::string filename = parser.getORcreateParam(std::string("DataOneMachine/bky20/bky20_1.txt"), "file", "filename", "-f", "GENERAL").value(); // Fichier à analyser valeur pas défaut bkY20_1
...
try
  {
    ...
    Parser parser_ = Parser(filename);
    Data data = parser_();
    
    // The fitness
    //////////////
    eoJobShopEvalFunc<Indi> plainEval(data) /* (varType  _anyVariable) */;
