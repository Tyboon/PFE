...
int main(int argc, char **argv)
{
  ...
  if((fp=fopen(argv[(argc == 5 ? 2 : 1)], "rb")))
  {
      ...
      for(i=0;i<nobjs;i++)
	error((fscanf(fp, "%lf", &(ubound[i]))!=1),"error in bound file");
      rewind(fp);
      do {
	  fscanf(fp, "%s", str);
      ...
      error(feof(fp), "error in bound file");
      for(i=0;i<nobjs;i++)
	error((fscanf(fp, "%lf", &(lbound[i]))!=1),"error in bound file");
       fclose(fp);
  }
  else
  {
...
int  determine_dim(FILE  *fp)
{
    ...
    no_obj = 0;
    line_found = 0;
    while (!line_found && fgets(line, MAX_LINE_LENGTH, fp) != NULL) // critical?
